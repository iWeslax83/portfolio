...
- Each of the three `ventures` entries renders as one bordered row: mono index (`01`/`02`/`03`), venture name (`font-display`), role, proof line (mono, same treatment as a catalogue row's tech-pills line), and an external link icon if `href` is present (Viyaro has none - render without the link affordance, not a disabled/greyed one). The icon-only link needs an `aria-label` (e.g. `` `${venture.name} - visit site` ``), same pattern `Log.tsx` already uses for its primary-link icon (`` `${project.title} - ${primary.label}` ``) - do not ship an unlabeled icon button.
- The Stratos row additionally nests its existing `stratosUnits` sub-list (quadrotor/FPV/VEX) compactly beneath its proof line - smaller type, same bordered-row idiom one level down. This is the only venture with a sub-list; Teluvane and Viyaro rows are single-row. On narrow viewports the sub-list stays a single column (stacked, full-width rows) rather than attempting a multi-column layout - it's three short items, a simple vertical list reads fine at any width and needs no responsive branching.
